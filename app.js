...
// Cold Start Loading Overlay State
...
async function loadFeed() {
    ...
    // Trigger Cold Start Overlay after 1.5s
...
async function processAndRenderPosts(posts, isMyPostsOnly) {
    ...
    // Direct render for instantaneous response (Fallback default interaction data)
    ...
    renderPosts(processedPosts, isMyPostsOnly);
}
...
function renderPosts(posts, isMyPostsOnly) {
    ...
    posts.forEach(post => {
        ...
        postCard.innerHTML = `
            ...
            <div class="mb-4">
                <p id="postContent-${post.id}" class="text-slate-300 text-sm whitespace-pre-line leading-relaxed line-clamp-4 transition-all duration-300">
                    ${post.content}
                </p>
                <button id="readMoreBtn-${post.id}" onclick="toggleReadMore(${post.id})" class="hidden text-xs text-cyan-400 hover:text-cyan-300 font-bold mt-2 flex items-center gap-1 cursor-pointer">
                    <span>View More</span> <i class="fa-solid fa-chevron-down text-[10px]"></i>
                </button>
            </div>
        ...
        const contentEl = document.getElementById(`postContent-${post.id}`);
        const readMoreBtn = document.getElementById(`readMoreBtn-${post.id}`);

        if (contentEl && readMoreBtn) {
            if (post.content && (post.content.length > 250 || post.content.split('\n').length > 4)) {
...
function toggleReadMore(postId) {
    ...
    const btn = document.getElementById(`readMoreBtn-${postId}`);

    if (!contentEl || !btn) return;

    if (contentEl.classList.contains("line-clamp-4")) {
        contentEl.classList.remove("line-clamp-4");
        btn.innerHTML = `<span>View Less</span> <i class="fa-solid fa-chevron-up text-[10px]"></i>`;
    } else {
        contentEl.classList.add("line-clamp-4");
        btn.innerHTML = `<span>View More</span> <i class="fa-solid fa-chevron-down text-[10px]"></i>`;
    }
}
...
async function handleUpdatePost() {
    ...
    try {
        ...
        if (data.success) {
            ...
            showCustomAlert(data.message || "Failed to update post!", "Error", true);
        }
    } catch (e) {
        showCustomAlert("Error updating post!", "Error", true);
    }
}
...
async function handleDeletePost(postId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            showCustomAlert(data.message || "Could not delete post.", "Error");
        }
    } catch (err) {
        showCustomAlert("Failed to delete post.", "Error");
    }
}
...
async function handleCreatePost() {
    ...
    if (!companyName || !title || !content) {
        return showCustomAlert("Please fill in all fields before submitting!", "Incomplete Form");
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            showCustomAlert(data.message || "Post rejected automatically: Content must be placement or career related!", "Post Auto-Rejected", true);
        }
    } catch (err) {
        showCustomAlert("Failed to submit post due to server issue!", "Server Error");
    }
}